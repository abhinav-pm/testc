...
class CorefDataProcessor:
    def __init__(self, config, language='telugu'):
        self.config = config
...
class Tensorizer:
    ...
    def tensorize_example(self, example, is_training):
        ...
        for idx, (sent_tokens, sent_speakers) in enumerate(zip(sentences, speakers)):
            ...
            # Padding
            padding_length = max_sentence_len - len(sent_input_ids)
            if padding_length > 0:
                sent_input_ids.extend([0] * padding_length)
                sent_input_mask.extend([0] * padding_length)
                sent_speaker_ids.extend([0] * padding_length)
            
            # Truncation if necessary
            if len(sent_input_ids) > max_sentence_len:
                sent_input_ids = sent_input_ids[:max_sentence_len]
                sent_input_mask = sent_input_mask[:max_sentence_len]
                sent_speaker_ids = sent_speaker_ids[:max_sentence_len]
        ...
        # Verify shapes
        assert input_ids.shape[1] == max_sentence_len
        assert input_mask.shape[1] == max_sentence_len
        assert speaker_ids.shape[1] == max_sentence_len
        ...
        example_tensor = (input_ids, input_mask, speaker_ids, sentence_len, genre, 
                        sentence_map, is_training, gold_starts, gold_ends, 
