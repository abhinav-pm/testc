...
class CorefModel(nn.Module):
    ...
    def get_predictions_and_loss(self, input_ids, input_mask, speaker_ids, sentence_len, genre, sentence_map,
        ...
        mention_doc = outputs['last_hidden_state']  # Extract the embeddings

        # Check if mention_doc is a tensor
        print(f"mention_doc type: {type(mention_doc)}")
        
        # Ensure mention_doc is a tensor before accessing its shape
        if isinstance(mention_doc, torch.Tensor):
            print(f"mention_doc shape: {mention_doc.shape}")
        else:
            print(f"Unexpected mention_doc value: {mention_doc}")
            return  # Exit early if mention_doc is invalid to prevent further errors
        
        # Convert input_mask to boolean
        input_mask = input_mask.to(torch.bool)
        
        # Apply mask to mention_doc and speaker_ids
        mention_doc = mention_doc[input_mask]
        speaker_ids = speaker_ids[input_mask]
        
        # Now get the number of words after masking
        ...
        if self.debug:
            if self.update_steps % 20 == 0:
                ...
                if conf['mention_loss_coef']:
                    # Analyze mention scores
                    gold_mention_scores = top_span_mention_scores[top_span_cluster_ids > 0]
                    non_gold_mention_scores = top_span_mention_scores[top_span_cluster_ids == 0]
                    
                    logger.info('Mention Detection:')
                    logger.info(f'  Gold mentions: {len(gold_mention_scores)}')
                    logger.info(f'  Non-gold mentions: {len(non_gold_mention_scores)}')
                    if len(gold_mention_scores) > 0:
                        logger.info(f'  Avg gold mention score: {gold_mention_scores.mean().item():.4f}')
                        logger.info(f'  Max gold mention score: {gold_mention_scores.max().item():.4f}')
                        logger.info(f'  Min gold mention score: {gold_mention_scores.min().item():.4f}')
                    if len(non_gold_mention_scores) > 0:
                        logger.info(f'  Avg non-gold mention score: {non_gold_mention_scores.mean().item():.4f}')
                    logger.info(f'  Mention loss: {loss_mention.item():.4f}')

                logger.info('\nCoreference Resolution:')
                if conf['loss_type'] == 'marginalized':
                    log_norm_mean = log_norm.mean().item()
                    log_marginalized_mean = log_marginalized_antecedent_scores.mean().item()
                    logger.info(f'  Avg log norm: {log_norm_mean:.4f}')
                    logger.info(f'  Avg log marginalized score: {log_marginalized_mean:.4f}')
                    logger.info(f'  Total coref loss: {(log_norm - log_marginalized_antecedent_scores).sum().item():.4f}')
                elif conf['loss_type'] == 'hinge':
                    logger.info(f'  Avg highest antecedent score: {highest_antecedent_scores.mean().item():.4f}')
                    logger.info(f'  Avg highest gold antecedent score: {highest_gold_antecedent_scores.mean().item():.4f}')
                    logger.info(f'  Avg hinge loss: {(slack_hinge * delta).mean().item():.4f}')
                    logger.info(f'  Total hinge loss: {loss.item():.4f}')

                if conf['higher_order'] == 'cluster_merging':
                    logger.info('\nCluster Merging:')
                    logger.info(f'  Cluster merging loss: {loss_cm.item():.4f}')

                # Antecedent analysis
                logger.info('\nAntecedent Analysis:')
                logger.info(f'  Max top antecedents: {max_top_antecedents}')
                logger.info(f'  Avg pairwise scores: {top_pairwise_scores.mean().item():.4f}')
                
                # Memory usage (if possible)
                if torch.cuda.is_available():
                    logger.info('\nGPU Memory:')
                    logger.info(f'  Allocated: {torch.cuda.memory_allocated() / 1024**2:.1f}MB')
                    logger.info(f'  Cached: {torch.cuda.memory_reserved() / 1024**2:.1f}MB')
